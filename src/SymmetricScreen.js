...
const SymmetricScreen = () => {
  ...
  const [isKeyExists, setIsKeyExists] = React.useState(false);
  const [accessLevel, setAccessLevel] = React.useState(0);
  const [invalidateOnNewBiometry, setInvalidateOnNewBiometry] =
    React.useState(false);
  const [alias, setAlias] = React.useState('test');
  const [textToBeEncrypted, setTextToBeEncrypted] = React.useState(
